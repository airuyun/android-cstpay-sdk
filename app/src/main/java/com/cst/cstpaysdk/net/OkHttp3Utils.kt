 ...
 * @cst_end
 */
class OkHttp3Utils private constructor() {
    ...
    /**
     * WebSocket长连接
     * @param webSocket WebSocket连接成功后得到的WebSocket对象
     * @param listener WebSocket事件监听
     * @param url 请求Url
     * @param jsonParams 请求的JSON字符串
     */
    fun doWebSocket(context: Context, webSocket: WebSocket?, listener: WebSocketListener, url: String, jsonParams: String) {
        Log.i("TJS", "======webSocket:url = $url")
        webSocket?.let {
            val sendSuccess = it.send(jsonParams)
            Log.i("TJS", "======webSocket:sendSuccess = $sendSuccess")
            return
        }
        webSocket?:let {
            val request = Request.Builder().url(url).build()
            getOkHttpClient(context)?.newWebSocket(request, listener)
        }
    }
    ...
    companion object {
        private var instance: OkHttp3Utils? = null
            get() {
                if (field == null) {
                    field = OkHttp3Utils()
                }
